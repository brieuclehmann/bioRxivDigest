...
def run_all_day_paper(
    ...
    print ("the date for the biorxiv data is: ", date)

    all_papers = [json.loads(l) for l in open(f"{data_dir}/{date}.jsonl", "r")]
    print (f"We found {len(all_papers)}.")

    ans_data = generate_relevance_score(all_papers, query, model_name, threshold_score, num_paper_in_prompt, temperature, top_p)
